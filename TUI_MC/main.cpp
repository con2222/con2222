#include <iostream>
#include <filesystem>
#include <fstream>
#include <ncurses.h>
#include <string>
#include <vector>

using namespace std::filesystem;
using namespace std;

vector<path> list_of_files() {
	vector<path> files;
	path current = current_path();
	for (const auto& dir_entry : directory_iterator(current)) {
        files.push_back(dir_entry.path());
    }
    return files;
}

string operations[6] = {"1. Open", "2. Rename", "3. Delete", "4. Copy", "5. Move", "6. File info"};

void draw_menu(WINDOW* win, const vector<path>& list, int highlight) {
    werase(win);
    box(win, 0, 0);
    mvwprintw(win, 0, 1, "Press ESC to exit");
    for(int i = 0; i < list.size(); i++) {
        if (i == highlight) {
            wattron(win, A_REVERSE);
        }
        mvwprintw(win, i + 1, 1, list[i].filename().string().c_str());
        if (i == highlight) {
            wattroff(win, A_REVERSE);
        }
    }
    wrefresh(win);
}

void draw_options(WINDOW* win, const string ops[], int size) {
    werase(win);
    box(win, 0, 0);
    mvwprintw(win, 0, 1, "Operations");
    for(int i = 0; i < size; i++) {
        mvwprintw(win, i + 1, 1, ops[i].c_str());
    }
    wrefresh(win);
}

void draw_file_info(WINDOW* win, path file) {
	werase(win);
	box(win, 0, 0);
	mvwprintw(win, 0, 1, "File info");
    string filename =  "Name: " + file.filename().string();
   	double filesize = file_size(file);
   	string size = "Size: " + to_string(filesize);
    mvwprintw(win, 1, 1, filename.c_str()); 
    mvwprintw(win, 2, 1, size.c_str()); 
    wrefresh(win);
}

void input_operation(WINDOW* optionwin, path file) {
    int choice;
    int flag = 0;
    
    int operation_highlight = 0;
    
    for(int i = 0; i < 5; i++) {
        if (i == operation_highlight) wattron(optionwin, A_REVERSE);
        mvwprintw(optionwin, i + 1, 1, operations[i].c_str());
        if (i == operation_highlight) wattroff(optionwin, A_REVERSE);
    }
    wrefresh(optionwin);
    
    while(true) {
        choice = wgetch(optionwin);
        
        switch(choice) {
            case KEY_UP:
                operation_highlight--;
                if (operation_highlight < 0) operation_highlight = 0;
                break;
            case KEY_DOWN:
                operation_highlight++;
                if (operation_highlight >= 6) operation_highlight = 5;
                break;
            case 27: // ESC
            	flag = 1;
            	operation_highlight = -1;
            case 10:
            	//mvwprintw(optionwin, 10, 1, operations[operation_highlight].c_str());
            	if (operations[operation_highlight] == "6. File info") {
            		while(true) {
            			draw_file_info(optionwin, file);
            			choice = wgetch(optionwin);
            			if (choice == 27) {
							draw_options(optionwin, operations, 6);
            				break;
            			}
            		}
            	}
            default:
                break;
        }
        
        for(int i = 0; i < 6; i++) {
            if (i == operation_highlight) wattron(optionwin, A_REVERSE);
            mvwprintw(optionwin, i + 1, 1, operations[i].c_str());
            if (i == operation_highlight) wattroff(optionwin, A_REVERSE);
        }
        wrefresh(optionwin);
        
        if (flag == 1) {
			break;
        }
    }
    
    draw_options(optionwin, operations, 6);
    return;
}



int main() {
	initscr();
	noecho();
	cbreak();
	curs_set(0);
	keypad(stdscr, TRUE);
	
	int yMax, xMax;
	getmaxyx(stdscr, yMax, xMax);
	int divider = xMax / 2;
	vector<path> list = list_of_files();
	
	WINDOW* menuwin = newwin(yMax, divider, 0, 0);
	WINDOW* optionwin = newwin(yMax, divider, 0, divider);
	keypad(menuwin, TRUE);
	
	int highlight = 0;
    int choice;
    
    draw_menu(menuwin, list, highlight);
    draw_options(optionwin, operations, 6);
    
	while(true) {
        choice = wgetch(menuwin);
        
        switch(choice) {
            case KEY_UP:
                highlight--;
                if (highlight < 0) highlight = 0;
                break;
            case KEY_DOWN:
                highlight++;
                if (highlight >= list.size()) highlight = list.size() - 1;
                break;
            case 10: // Enter
                {
                    string msg = "Selected: " + list[highlight].filename().string();
                    mvwprintw(optionwin, 7, 1, msg.c_str());
                    wrefresh(optionwin);
					
					wmove(optionwin, 0, 1);
					wrefresh(optionwin);
					
					keypad(optionwin, TRUE);
					input_operation(optionwin, list[highlight]);
					keypad(optionwin, FALSE);
					
					touchwin(menuwin);
    				wrefresh(menuwin);
					break;
                }
                break;
            case 27: // ESC
                endwin();
                return 0;
            default:
                break;
    	}
    	draw_menu(menuwin, list, highlight);
    }
	
	delwin(menuwin);
	delwin(optionwin);
	endwin();
	return 0;
}
